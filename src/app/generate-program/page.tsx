...
function GenerateProgramPage() {
  ...
  // navigate user to profile page after call end
  useEffect(() => {
    if (callEnded) {
      const rediretTimer = setTimeout(() => {
        router.push('./profile')
      }, 1500)
      return () => clearTimeout(rediretTimer)
    }
  }, [callEnded, router])

  // set up evet lisner for VAPI
  useEffect(() => {
